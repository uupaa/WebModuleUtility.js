...
function _getBootediOSSimulatorUUID(callback, errorCallback) {
    _exec("xcrun simctl list | grep Booted", function(stdout) {
        ...
        if (line) {
            return line.split(")")[0].split("(")[1]; // E6AA0287-6C06-4F03-A61E-C96B75B587CD
        }
        return "";
    }, errorCallback);
}
...
function WebModuleUtility_killsim(callback, errorCallback) {
    _getBootediOSSimulatorUUID(); // TBD

    _exec("xcrun simctl list | grep Booted", callback, errorCallback);
}
